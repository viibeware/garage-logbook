...
function openMaintDetail(e){document.getElementById('viewMaintTitle').textContent=e.title;document.getElementById('viewMaintContent').innerHTML=`<div class="view-maint-details"><div class="detail-item"><span class="lbl">Type</span><span class="val">${e.maintenance_type}</span></div><div class="detail-item"><span class="lbl">Date</span><span class="val">${formatDate(e.service_date)}</span></div>${e.odometer?`<div class="detail-item"><span class="lbl">Odometer</span><span class="val">${Number(e.odometer).toLocaleString()} mi</span></div>`:''}${e.parts_vendor?`<div class="detail-item"><span class="lbl">Vendor</span><span class="val">${esc(e.parts_vendor)}</span></div>`:''}${e.cost!=null?`<div class="detail-item"><span class="lbl">Cost</span><span class="val">$${Number(e.cost).toFixed(2)}</span></div>`:''}</div>${e.notes?`<div class="view-maint-notes">${esc(e.notes)}</div>`:''}${e.images&&e.images.length?`<div class="view-maint-gallery">${e.images.map((img,idx)=>`<img src="/uploads/maintenance/${img.filename}" alt="" onclick="event.stopPropagation();openLightbox(${JSON.stringify(e.images.map(i=>'/uploads/maintenance/'+i.filename)).replace(/"/g,'&quot;')},${idx})">`).join('')}</div>`:''}`;document.getElementById('viewMaintActions').innerHTML=canEdit()?`<button class="btn btn-danger btn-sm" onclick="deleteMaintenance(${e.id})">Delete</button><button class="btn btn-ghost btn-sm" onclick="duplicateMaintenance(${e.id})">Duplicate</button><button class="btn btn-primary btn-sm" onclick="closeModal('viewMaintModal');openEditMaintModal(${e.id})">Edit</button>`:`<button class="btn btn-ghost" onclick="closeModal('viewMaintModal')">Close</button>`;openModal('viewMaintModal')}

function openAddMaintenanceModal(){document.getElementById('addMaintForm').reset();document.getElementById('galleryThumbs').innerHTML='';resetPreview('maintGalleryPreview');document.querySelector('#addMaintForm [name="service_date"]').valueAsDate=new Date();openModal('addMaintModal')}

async function submitMaintenance(e){e.preventDefault();if(!currentCarId)return;const b=document.getElementById('addMaintBtn');b.disabled=true;b.textContent='Saving…';try{const r=await fetch('/api/cars/'+currentCarId+'/maintenance',{method:'POST',body:new FormData(e.target)});if(!r.ok)throw new Error((await r.json()).error);toast('Saved!','success');closeModal('addMaintModal');loadMaintenance();loadDashboard()}catch(e){toast(e.message,'error')}finally{b.disabled=false;b.textContent='Save Record'}}
...
async function submitEditMaintenance(e){e.preventDefault();const id=document.getElementById('editMaintId').value;try{const r=await fetch('/api/maintenance/'+id,{method:'PUT',body:new FormData(e.target)});if(!r.ok)throw new Error((await r.json()).error);toast('Updated','success');closeModal('editMaintModal');loadMaintenance();loadDashboard()}catch(e){toast(e.message,'error')}}
